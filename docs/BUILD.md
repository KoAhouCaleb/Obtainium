...
| `Class 'app.revanced.patcher.patch.*' was compiled by a pre-release version of Kotlin and cannot be loaded by this version of the compiler` | `app.revanced:patcher-android:22.0.2-dev.1` is a pre-release snapshot compiled by a newer Kotlin compiler. `android/app/build.gradle.kts`'s `kotlinOptions` already adds `-Xskip-prerelease-check` for this. If a stable (non-`-dev`) `patcher-android` release exists by the time you're reading this, prefer switching to that and removing the flag over relying on it indefinitely. |
| `mergeNormalReleaseJavaResource` (or similar) fails with `N files found with path 'META-INF/...'` | Several of `revanced-patcher`/`revanced-library`'s dependencies (Apache Tika, httpclient/httpcore, smali/baksmali, bouncycastle, jspecify) ship duplicate `META-INF`/license/properties/OSGi-manifest paths. `android/app/build.gradle.kts`'s `packaging { resources { excludes += ... } }` block covers the known set (mirrored from revanced-manager's own build config, plus a glob for multi-release JARs' per-version `OSGI-INF/**` manifests, which Android has no use for regardless); if a *new*, differently-shaped duplicate path shows up, add it there too. |
| `minifyNormalReleaseWithR8` fails with "Missing classes detected" (`java.awt.*`, `org.immutables.*`, `reactor.blockhound.*`, etc.) | Expected — R8/minification is intentionally **disabled** for the release build type (`isMinifyEnabled = false` in `android/app/build.gradle.kts`), for the same reason revanced-manager's own build disables it: "Causes patching to not work properly, if enabled" - R8 can silently break `revanced-patcher`'s reflection-based class loading even once the missing-class warnings are suppressed. If you see this error, minification got re-enabled somewhere (e.g. a merge/rebase, or an explicit `--` build flag) - turn it back off rather than chasing individual `-dontwarn` rules. |
