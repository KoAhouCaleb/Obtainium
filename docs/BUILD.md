...
| `mergeNormalReleaseJavaResource` (or similar) fails with `N files found with path 'META-INF/...'` | Several of `revanced-patcher`/`revanced-library`'s dependencies (Apache Tika, httpclient/httpcore, smali/baksmali, bouncycastle) ship duplicate `META-INF`/license/properties paths. `android/app/build.gradle.kts` already has a `packaging { resources { excludes += ... } }` block covering the known set (mirrored from revanced-manager's own build config, which hits the same conflicts); if a *new* duplicate path shows up that isn't in that list yet, add it there. |
